...
Each reference section below places its actual output beside the command that
produces it, on both light and dark transcript backgrounds. The previews are
generated by running `birc-text-effects.js`, not by manually approximating its
output. Glyph shape and IRC colors can still vary with client fonts and
palettes.
...
![Word transformation commands with light and dark output](previews/word-transformations.svg)
...
![Unicode novelty commands with light and dark output](previews/unicode-novelty-forms.svg)

Each preview output has its own SVG clipping rectangle. Zalgo receives
additional vertical padding and line height within both its light and dark
boundaries, preventing combining marks from spreading into labels or adjacent
examples.
...
![All IRC foreground colors with light and dark output](previews/irc-foreground-colors-indexes-00-15.svg)

![All IRC background colors with light and dark output](previews/irc-background-colors-indexes-00-15.svg)

![IRC character formatting with light and dark output](previews/irc-character-formatting.svg)
...
![Named IRC color schemes with light and dark output](previews/named-irc-color-schemes.svg)
...
![Every ANSI color option with light and dark output](previews/visible-ansi-escape-notation.svg)
...
![Box and block font commands with light and dark output](previews/boxes-and-block-lettering.svg)
...
Regenerate the README previews after changing an effect, option, font, color
scheme, or example:

```sh
npm run build:text-effects-previews
```
