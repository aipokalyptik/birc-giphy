...
The following generated previews show every transformation and option on both
light and dark transcript backgrounds. They include all leet and Zalgo levels,
all bundled schemes, every IRC foreground and background index, all IRC and
ANSI formatting choices, every block font in normal and solid modes, and the
command aliases that produce output.

The visible result is generated by running the documented command through
`birc-text-effects.js`; it is not a hand-written approximation. Actual glyph
shape and IRC color appearance can still vary with the receiving client's font
and palette.

<details>
<summary>Light-background examples</summary>

![Every bIRC text effect on a light background](previews/text-effects-light.svg)

</details>

<details>
<summary>Dark-background examples</summary>

![Every bIRC text effect on a dark background](previews/text-effects-dark.svg)

</details>

Every example has its own SVG clipping rectangle. Zalgo examples receive extra
vertical padding and line height inside that boundary, preventing their
combining marks from spreading into labels or adjacent examples.
